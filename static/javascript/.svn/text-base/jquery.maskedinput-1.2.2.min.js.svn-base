...
(function(a){
	...
	a.mask={definitions:{"1":"[0-1]","3":"[0-3]","9":"[0-9]",a:"[A-Za-z]","*":"[A-Za-z0-9]"}};
	a.fn.extend({
		caret:function(e,f){
		...
		,mask:function(j,d){
			...
			return this.each(function(){
				var r=a(this);
				var m=a.map(j.split(""),function(x,y){if(x!="?"){return k[x]?d.placeholder:x}});
				var n=false;
				var q=r.val();
				...
				if(!r.attr("readonly")){
					r.one("unmask",function(){
						...
						setTimeout(function(){r.caret(p(true))}
							,0)
					})
				}
				p()
